...
class WorkerBridge(worker_interface.WorkerBridge):
    ...
    def __init__(self, node, my_address, donation_percentage, merged_urls,
        ...
        self.tracker_view = forest.TrackerView(self.node.tracker, forest.get_attributedelta_type(dict(forest.AttributeDelta.attrs,
                                                                                                      ...
                                                                                                      my_orphan_announce_count=lambda share: 1 if share.hash in self.my_share_hashes and share.share_data[
                                                                                                          'stale_info'] == 'orphan' else 0,
                                                                                                      my_dead_announce_count=lambda share: 1 if share.hash in self.my_share_hashes and share.share_data[
        ...
        def set_merged_work(merged_url, merged_userpass):
            merged_proxy = jsonrpc.HTTPProxy(merged_url, dict(
                Authorization='Basic ' + base64.b64encode(merged_userpass)))
            while self.running:
                ...
                self.merged_work.set(math.merge_dicts(self.merged_work.value, {auxblock['chainid']: dict(
                    ...
                    target='p2pool' if target == 'p2pool' else pack.IntType(
                        256).unpack(target.decode('hex')),
    ...
    def get_stale_counts(self):
        ...
        my_shares_in_chain = delta.my_count + \
            self.removed_unstales_var.value[0]
    ...
    def get_local_rates(self):
        ...
        for datum in datums:
            ...
            if datum['dead']:
                miner_dead_hash_rates[datum['user']] = miner_dead_hash_rates.get(
                    datum['user'], 0) + datum['work']/dt
    ...
    def get_work(self, user, address, desired_share_target,
        ...
        unknown_rules = set(r[1:] if r.startswith('!') else r for r in self.node.bitcoind_work.value['rules']
                            if r) - set(getattr(self.node.net, 'SOFTFORKS_REQUIRED', []))
        if unknown_rules:
            print "Unknown softforks found: ", unknown_rules
            print "Required softforks: ", self.node.bitcoind_work.value['rules'], "\tKnown: ", getattr(
                self.node.net, 'SOFTFORKS_REQUIRED', [])
        ...
        if self.merged_work.value:
            ...
            mm_hashes = [self.merged_work.value.get(tree.get(i), dict(hash=0))[
                'hash'] for i in xrange(size)]
        ...
        previous_share = self.node.tracker.items[
            self.node.best_share_var.value] if self.node.best_share_var.value is not None else None
        if previous_share is None:
            ...
            if previous_share_type.SUCCESSOR is None or self.node.tracker.get_height(previous_share.hash) < self.node.net.CHAIN_LENGTH:
                ...
                upgraded = counts.get(
                    successor_type.VERSION, 0)/sum(counts.itervalues())
                if upgraded > .65:
        ...
        if True:
            share_info, gentx, other_transaction_hashes, get_share = share_type.generate_transaction(
                ...
                share_data=dict(
                    ...
                    desired_version=(
                        share_type.SUCCESSOR if share_type.SUCCESSOR is not None else share_type).VOTING_VERSION,
                ),
        ...
        other_transactions = [tx_map[tx_hash]
                              for tx_hash in other_transaction_hashes]

        del tx_hashes
        if self.node.cur_share_ver >= 34:
            tx_map = {}  # we can free up this memory now

        mm_later = [(dict(aux_work, target=aux_work['target'] if aux_work['target'] !=
                          'p2pool' else share_info['bits'].target), index, hashes) for aux_work, index, hashes in mm_later]
        ...
        merkle_link = bitcoin_data.calculate_merkle_link([None] + other_transaction_hashes, 0) if share_info.get(
            'segwit_data', None) is None else share_info['segwit_data']['txid_merkle_link']
        ...
        if print_throttle is 0.0:
            ...
            if (current_time - print_throttle) > 5.0:
                print 'New work! Share diff: %.02f Block value: %.2f %s (%i tx, %.0f kB)' % (
                    ...
                    1e-8, self.node.net.PARENT.SYMBOL,
                    len(self.current_work.value['transactions']),
                    sum((bitcoin_data.get_size(tx)
                         for tx in self.current_work.value['transactions']))/1000.,
                )
                print_throttle = time.time()
        ...
        def got_response(header, username, coinbase_nonce, pseudoshare_target):
            ...
            new_packed_gentx = packed_gentx[:-self.COINBASE_NONCE_LENGTH-4] + coinbase_nonce + \
                packed_gentx[-4:] if coinbase_nonce != '\0' * \
                self.COINBASE_NONCE_LENGTH else packed_gentx
            new_gentx = bitcoin_data.tx_type.unpack(
                new_packed_gentx) if coinbase_nonce != '\0'*self.COINBASE_NONCE_LENGTH else gentx
            ...
            try:
                if pow_hash <= header['bits'].target or p2pool.DEBUG:
                    if pow_hash <= header['bits'].target:
                        ...
                        print
                    helper.submit_block(
                        dict(header=header, txs=[new_gentx] + other_transactions), False, self.node)
            except:
            ...
            if pow_hash <= share_info['bits'].target and header_hash not in received_header_hashes:
                ...
                if self.node.cur_share_ver < 34:
                    ...
                    missing = {hsh: val for (
                        hsh, val) in tx_map.iteritems() if not hsh in known_txs}
                    if missing:
                ...
                self.share_received.happened(bitcoin_data.target_to_average_attempts(
                    share.target), not on_time, share.hash)
            ...
            if pow_hash > pseudoshare_target:
                ...
                self.recent_shares_ts_work.append(
                    (time.time(), bitcoin_data.target_to_average_attempts(pseudoshare_target)))
                while len(self.recent_shares_ts_work) > 50:
                    self.recent_shares_ts_work.pop(0)
                self.local_rate_monitor.add_datum(dict(work=bitcoin_data.target_to_average_attempts(
                    pseudoshare_target), dead=not on_time, user=username, share_target=share_info['bits'].target))
                self.local_addr_rate_monitor.add_datum(dict(
                    work=bitcoin_data.target_to_average_attempts(pseudoshare_target), address=address))
            t1 = time.time()
