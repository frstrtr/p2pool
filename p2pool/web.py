...
def get_web_root(wb, datadir_path, bitcoind_getinfo_var, stop_event=variable.Event(), static_dir=None):
    ...
    def get_current_scaled_txouts(scale, trunc=0):
        ...
        results = dict((addr, value*scale//total)
                       for addr, value in txouts.iteritems())
        if trunc > 0:
            total_random = 0
            random_set = set()
            ...
            if total_random:
                winner = math.weighted_choice(
                    (addr, results[script]) for addr in random_set)
    ...
    new_root.putChild('my_share_hashes', WebInterface(lambda: [
                      '%064x' % my_share_hash for my_share_hash in list(sorted(wb.my_share_hashes,key = SortShareHashesByTimestamp, reverse=True))[:]]))
