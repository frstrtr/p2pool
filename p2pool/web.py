...
def get_web_root(wb, datadir_path, bitcoind_getinfo_var, stop_event=variable.Event(), static_dir=None):
    ...
    web_root.putChild('recent_blocks', WebInterface(lambda: [dict(
        ...
        share='%064x' % s.hash,
        #Extending last blocks mined to be displayed for period of time = CHAIN_LENGTH*100
    ) for s in node.tracker.get_chain(node.best_share_var.value, min(node.tracker.get_height(node.best_share_var.value), node.net.CHAIN_LENGTH*100)) if s.pow_hash <= s.header['bits'].target]))
    ...
    hd = graph.HistoryDatabase.from_obj({
        ...
        'miner_hash_rates': graph.DataStreamDescription(dataview_descriptions, is_gauge=False, multivalues=True, multivalues_keep=10000), # Better graphing UI for workers multivalues_keep=10000
        'miner_dead_hash_rates': graph.DataStreamDescription(dataview_descriptions, is_gauge=False, multivalues=True, multivalues_keep=10000), # Better graphing UI for workers multivalues_keep=10000
