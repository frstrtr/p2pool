...
hash_link_type = pack.ComposedType([
    ...
    # bit of a hack, but since the donation script is at the end, const_ending is long enough to always make this empty
    ('extra_data', pack.FixedStrType(0)),
...
class WeightsSkipList(forest.TrackerSkipList):
    ...
    def apply_delta(self, (share_count1, weights_list, total_weight1, total_donation_weight1), (share_count2, weights2, total_weight2, total_donation_weight2), (max_shares, desired_weight)):
        if total_weight1 + total_weight2 > desired_weight and share_count2 == 1:
            ...
            new_weights = {script: (
                desired_weight - total_weight1)//65535*weights2[script]//(total_weight2//65535)}
...
class OkayTracker(forest.Tracker):
    ...
    def think(self, block_rel_height_func, block_abs_height_func, previous_block, bits, known_txs, feecache):
                ...
                if last is not None:
                    desired.add((
                        self.items[random.choice(
                            list(self.reverse[last]))].peer_addr,
                        last,
        ...
        decorated_tails = sorted((self.score(max(
            self.verified.tails[tail_hash], key=self.verified.get_work), block_rel_height_func), tail_hash) for tail_hash in self.verified.tails)
        if p2pool.DEBUG:
            print len(decorated_tails), 'tails:'
            for score, tail_hash in decorated_tails:
                print format_hash(tail_hash), score
        best_tail_score, best_tail = decorated_tails[-1] if decorated_tails else (
            None, None)

        # decide best verified head
        decorated_heads = sorted(((
            self.verified.get_work(self.verified.get_nth_parent_hash(h, min(5, self.verified.get_height(h)))) -
            min(self.items[h].should_punish_reason(previous_block, bits, self, known_txs)[
                0], 1) * bitcoin_data.target_to_average_attempts(self.items[h].target),
        ...
        traditional_sort = sorted(((
            ...
            #self.items[h].peer_addr is None,
            # assume they can't tell we should punish this share and will be sorting based on time
            -self.items[h].time_seen,
            -self.items[h].should_punish_reason(previous_block, bits, self, known_txs)[0],
        ), h) for h in self.verified.tails.get(best_tail, []))
        ...
        if best is not None:
            ...
            while punish > 0:
                print 'Punishing share for %r! Jumping from %s to %s!' % (
                    punish_reason, format_hash(best), format_hash(best_share.previous_hash))
                ...
                if not punish:
                    def best_descendent(hsh, limit=20):
                        ...
                        best_kids = sorted((best_descendent(
                            child, limit-1) for child in child_hashes if not self.items[child].naughty))
        ...
        if p2pool.DEBUG:
            print 'Desire %i shares. Cutoff: %s old diff>%.2f' % (len(desired), math.format_dt(
                time.time() - timestamp_cutoff), bitcoin_data.target_to_difficulty(target_cutoff))
            for peer_addr, hash, ts, targ in desired:
                print '   ', None if peer_addr is None else '%s:%i' % peer_addr, format_hash(hash), math.format_dt(
                    time.time() - ts), bitcoin_data.target_to_difficulty(targ), ts >= timestamp_cutoff, targ <= target_cutoff
...
def get_pool_attempts_per_second(tracker, previous_share_hash, dist, min_work=False, integer=False):
    ...
    attempts = tracker.get_delta(near.hash, far.hash).work if not min_work else tracker.get_delta(
        near.hash, far.hash).min_work
...
def get_average_stale_prop(tracker, share_hash, lookbehind):
    stales = sum(1 for share in tracker.get_chain(
        share_hash, lookbehind) if share.share_data['stale_info'] is not None)
...
def get_stale_counts(tracker, share_hash, lookbehind, rates=False):
    ...
    for share in tracker.get_chain(share_hash, lookbehind - 1):
        res['good'] = res.get(
            'good', 0) + bitcoin_data.target_to_average_attempts(share.target)
        s = share.share_data['stale_info']
        if s is not None:
            res[s] = res.get(
                s, 0) + bitcoin_data.target_to_average_attempts(share.target)
    if rates:
...
def get_expected_payouts(tracker, best_share_hash, block_target, subsidy, net):
    weights, total_weight, donation_weight = tracker.get_cumulative_weights(best_share_hash, min(tracker.get_height(
        best_share_hash), net.REAL_CHAIN_LENGTH), 65535*net.SPREAD*bitcoin_data.target_to_average_attempts(block_target))
    ...
    res[donation_addr] = res.get(
        donation_addr, 0) + subsidy - sum(res.itervalues())
    return res
...
def get_desired_version_counts(tracker, best_share_hash, dist):
    ...
    for share in tracker.get_chain(best_share_hash, dist):
        res[share.desired_version] = res.get(
            share.desired_version, 0) + bitcoin_data.target_to_average_attempts(share.target)
    return res
...
def get_warnings(tracker, best_share, net, bitcoind_getinfo, bitcoind_work_value):
    ...
    majority_desired_version = max(
        desired_version_counts, key=lambda k: desired_version_counts[k])
    ...
    version_warning = getattr(net, 'VERSION_WARNING', lambda v: None)(
        bitcoind_getinfo['version'])
...
# Read and write shares from files
...
class ShareStore(object):
                    ...
                    try:
                        ...
                        if type_id == 0:
                            ...
                            raise NotImplementedError(
                                "share type %i" % (type_id,))
                    except Exception:
                        log.err(
                            None, "HARMLESS error while reading saved shares, continuing where left off:")

        # filename -> (set of share hashes, set of verified hashes)
        self.known = known
        self.known_desired = dict((k, (set(a), set(b)))
                                  for k, (a, b) in known.iteritems())

        print "Share loading took %.3f seconds" % (time.time() - start)

    # Save shares to files db
    ...
    # Purge shares out of chain length?
