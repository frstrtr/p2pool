...
class HeightTracker(object):
    ...
    def _think(self):
        try:
            highest_head = max(self._tracker.heads, key=lambda h: self._tracker.get_height_and_last(
                h)[0]) if self._tracker.heads else None
...
def get_height_funcs(bitcoind, factory, best_block_func, net):
    ...
    if '\ngetblockheader ' in (yield deferral.retry()(bitcoind.rpc_help)()):
        ...
        get_height_rel_highest = HeightTracker(
            best_block_func, factory, 5*net.SHARE_PERIOD*net.CHAIN_LENGTH/net.PARENT.BLOCK_PERIOD).get_height_rel_highest
        get_height = HeightTracker(
            best_block_func, factory, 5*net.SHARE_PERIOD*net.CHAIN_LENGTH/net.PARENT.BLOCK_PERIOD).get_height
