    ...
    for n in share_json:
        if n == 'share_data':
            ...
            for n1 in share_json[n]:
                ...
                if n1 == 'timestamp':
                    print '\t', n1, 'human friendly:', datetime.utcfromtimestamp(share_json[n][n1]).strftime('%Y-%m-%d %H:%M:%S'),'\n'
        elif n == 'local':
            print 'LOCAL:', '\n'
            for n2 in share_json[n]:
                print '\t', n2, ':', share_json[n][n2], '\n'
                if n2 == 'time_first_seen':
                    print '\t', n2, 'human friendly:', datetime.utcfromtimestamp(share_json[n][n2]).strftime('%Y-%m-%d %H:%M:%S'),'\n'
            ...
            for n3 in share_json[n]:
                if n3 == 'header':
                    ...
                    for n4 in share_json[n][n3]:
                        ...
                        if n4 == 'timestamp':
                            print '\t\t', n4, 'human friendly:', datetime.utcfromtimestamp(share_json[n][n3][n4]).strftime('%Y-%m-%d %H:%M:%S'),'\n'
                elif n3 == 'gentx':
                    print '\t', 'GENTX:', '\n'
                    for n4 in share_json[n][n3]:
                        if n4 == 'coinbase':
